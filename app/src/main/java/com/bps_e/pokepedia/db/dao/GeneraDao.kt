...
import com.bps_e.pokepedia.db.Poke.Genera as T

@Dao
interface GeneraDao : DatabaseConnector<T> {
    companion object { const val TABLE = Poke.TABLE_GENERA }

    @Query("SELECT * FROM $TABLE WHERE name_id = :id")
    fun getLocal(id: Int): List<Poke.Genera>
